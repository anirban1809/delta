...
class BoxedCounter {
    ...
    public mod step(): void | OverflowError {
...
- `borrowed heap T` and `mod borrowed heap T` parameters — the borrow points to the heap pointer, auto-deref still works through the borrow.
...
- The borrow machinery doesn't auto-deref through `heap T`.
...
6. **Borrows through `heap T`.** `borrowed heap T` lowers to `T* const *` (a non-mutable pointer to a non-mutable heap pointer; the indirection levels reflect "borrow of a heap pointer"). For ergonomics, member access auto-derefs both levels: `bp.field` lowers to `(*bp)->field`. The capability checks compose: `borrowed heap T` may not call `mod` methods; `mod borrowed heap T` may.
...
- **Borrow through heap.** A `borrowed heap T` parameter resolves member access by auto-derefing through both indirection levels. Capability check unchanged.
...
- **Borrowed-heap lowering.** `borrowed heap T` → `T* const *`. Access `bp.field` → `(*bp)->field`. The two derefs add nothing at runtime (clang folds).
...
- `auto_deref_field_write_ok` — write through heap field (`mod` method).
...
**Borrows through heap (2)**
- `borrowed_heap_field_read_ok` — `borrowed heap Payload` argument; read through.
- `mod_borrowed_heap_mod_method_ok` — `mod borrowed heap Payload`; call a `mod` method.
...
7. Analyzer: borrow-through-heap (member access via `borrowed heap T`).
...
11. Codegen: borrowed-heap two-level deref lowering.
...
- **Borrow exclusivity through heap.** `f(mod borrowed bc.payload, borrowed bc.payload)` — the root for exclusivity purposes is `bc`. Both borrow operands share the root `bc` (the path `bc.payload` derefs the heap pointer but the root is still `bc`). The exclusivity check rejects, correctly. Verified by `borrow_field_root_check_ok` from Phase G with the heap variant added.
