...
The user's `function main(): int32` becomes:
...
If the source has no `main`, or `main` has parameters, or `main` doesn't return `int32`, codegen emits a diagnostic and stops. The exact validation rules belong in the analyzer eventually ([§1.5](../spec-sections/01-source-file-convention.md)) but v0 codegen enforces them at the codegen boundary.
...
| File | Notes |
|---|---|
| `return_literal_ok.delta` | `function main(): int32 { return 42; }` → exit 42 |
...
4. **Return statements + integer expressions.** Now `function main(): int32 { return 42; }` compiles and runs end-to-end. This is the milestone where the rest of the work is just filling in more node types.
